...
The workspace contains a root README, nine learning-category directories with README indexes, and a runnable TypeScript travel-planner comparison at `agents/travel-planner-gemini-typesafe/`. It includes a LangGraph agent backend, offline fixture evaluation, live Gemini Google Search grounding and TypeSafe adapters, tests, a scenario evaluator, two controlled benchmark runners, and local run history in the Git-ignored `.runs/` directory. Local web-grounded smoke comparisons, foundational and challenge frozen-state routing benchmarks, and a frozen-evidence whole-agent benchmark have completed; their sanitized results are under `benchmarks/results/` in the example. A Vercel Node-function adapter and private Blob run-history store are available; public-deployment review remains outstanding. Run `npm ci` and then its commands from the project directory (`npm run check`, `npm test`, `npm run eval`, `npm run benchmark`, `npm run benchmark:agent`, `npm start`). The benchmark scripts default to a no-API dry run; `--live` makes paid calls. Contribution conventions are in the root README and this guide; a standalone contribution guide and a root `.gitignore` are available. A license and CI have not been established. There is no repository-wide install, build, lint, or test command. Inspect actual files before assuming a toolchain or command exists.
